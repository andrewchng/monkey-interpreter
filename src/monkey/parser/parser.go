...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerInfix(token.PLUS, p.parseInfixExpresion)
	p.registerInfix(token.MINUS, p.parseInfixExpresion)
	p.registerInfix(token.SLASH, p.parseInfixExpresion)
	p.registerInfix(token.ASTERISK, p.parseInfixExpresion)
	p.registerInfix(token.EQ, p.parseInfixExpresion)
...
func (p *Parser) parseBlockStatement() *ast.BlockStatement {
	blkStatement := &ast.BlockStatement{Token: p.curToken}
	blkStatement.Statements = []ast.Statement{}
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		...
		if stmt != nil {
			blkStatement.Statements = append(blkStatement.Statements, stmt)
		}
		p.nextToken()
	}
...
func (p *Parser) parseInfixExpresion(left ast.Expression) ast.Expression {
